function loadOnProducts(){
    ...
    fetch('http://localhost:8060/products/getAll', {
        method: 'GET',
        headers: {
        ...
        if (response.ok) {
           let data = await response.json();
            ...
            data.products.forEach(element => {
                ...
                <td><button type="button" class="btn btn-danger delete-btn" data-id="${element.id}" style ="font-size:7px">Delete</td>
                </tr>
...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('delete-btn')) {
        ...
        if (confirm('Are you sure to delete this product?')) {
            fetch(`http://localhost:8060/products/delete/${productId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
